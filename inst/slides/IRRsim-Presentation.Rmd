...
* Writing is one of the assessments in DAACS. We were trying to establish inter-rater reliability in two ways:
	1. Human-to-human
	2. Human-to-machine
* Literature clearly suggests ICC is the appropriate measure, however most of that literature is from the medical literature.
* Guidance on interpretation of ICC is not very clear.

## {.flexbox .vcenter}

<p align="center"><font size="12">Communicating with the public<br/><br/>about educational measurement.</font></p>

<div align="right"><font size="4">NCME 2019 Conference Theme</font></div>
...
In education, ICC(1,1) is *often* the most appropriate intra-class correlation (ICC) metric to use given each subject is measured by two randomly selected raters (one-way), single measures are used, and absolute agreement is desired.

Guidelines for interpreting ICC is largely provided from the medical literature. When discussing to ICC(1,1), Koo and Li (2016) state that **"practically, this model is rarely used in clinical reliability analysis because majority of the reliability studies typically involve the same set of raters to measure all subjects"** (pp. 156-157).
...
## Modeling ICC from Percent Agreement {.columns-2}
...
Simulated `r prettyNum(nrow(IRRsimData), big.mark = ',')` scoring matrices<footnote>The results are included in the package and can be loaded using the `data(IRRsimData)` command. The script to generate these results is available here: https://github.com/jbryer/IRRsim/blob/master/data-raw/IRRsimulations.R </footnote> (each 100 x *k*) with *k* between 2 and 16 raters and four response distributions (i.e. uniform, lightly skewed, moderately skewed, and highly skewed).


For each scoring desing (i.e. $k$, $k_m$, & number of scoring levels), the following model was estimated: 

$$ICC = \beta_{PRA}^2 + \beta_{PRA} + B_0$$

For ICC(1,1), ICC(2,1), and ICC(3,1), $R^2 \ge 0.82$
...
* When $k_m = 2$, there is a penalty in ICC as *k* increases.
* The published guidelines for interpreting ICC may not be appropriate for one-way designs (i.e. ICC(1,1) and ICC(1,k)). For many common scoring designs (e.g. $k_m = 2$, $k > 20$) in education, achieving "excellent" inter-rater reliability is not possible.
...
### Website: http://irrsim.bryer.org/
### Source Code: https://github.com/jbryer/IRRsim
